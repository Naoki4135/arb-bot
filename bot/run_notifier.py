...
from bot.strategy import jpy_ratio, notify_needed  # 価格比計算と通知条件判定
        ...
        if prices:  # 価格が取得できた場合
            ...
            for base, p in prices.items():  # 通貨ごとに処理
                ...
                print(
                    ...
                    f"BITBANK(¥): {p['bitbank_bid_jpy']:.4f}  RATIO: {ratio:.4f}"
                )  # 取得した価格をログ出力
                info = last[base]  # 直近通知情報を取り出す
                ok, reason = notify_needed(ratio, info['at'], info['ratio'], t0)  # 通知が必要か判定
                if ok:  # 通知すべきなら
                    msg = (
                        f"🚀 **裁定チャンス ({base})**\n"  # 通知メッセージ作成
                        f"RATIO: `{ratio:.4f}`\n"
